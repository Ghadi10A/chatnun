...
def predict_signals(request):
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            context = {'form': form, 'close_price': round(close_price, 2), 'signal': signal, 'vwap': round(vwap, 2), 'ticker': ticker, 'accuracy': round(accuracy), 'last_diff': round(last_diff, 2), 'last_diff_percent': round(last_diff_percent, 2), 'data': data, 'articles': articles, 'new_conversation_id': new_conversation_id, 'LANGUAGES': settings.LANGUAGES}
...
def show_profile(request, username):
    ...
    if request.user.is_authenticated:
        if request.user.username == username:  # check if the user is visiting their own profile
            ...
            if following.exists():
                is_following = True            
            return render(request, 'public_profile.html', {'user': user, 'is_following': is_following, 'notifications': notifications, 'unread_notifications': unread_notifications, 'new_conversation_id': new_conversation_id, 'LANGUAGES': settings.LANGUAGES})
